    ...
    for(float i = lmin; i <= rmax; i++){
        for(float j = bmin; j <= tmax; j++){//遍历bounding box像素
            ...
            if(insideTriangle(i+0.5, j+0.5, t.v)){//如果像素在三角形内
                // If so, use the following code to get the interpolated z value.加权平均三角形顶点的深度Z
                std::tie(alpha, beta, gamma) = computeBarycentric2D(i+0.5, j+0.5, t.v);
                float w_reciprocal = 1.0/(alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w()); //倒数加权
                float z_interpolated = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                z_interpolated *= w_reciprocal;
                    
                if (-z_interpolated < depth_buf[id]){//如果该像素的深度更小，四个插值颜色、法向量、纹理三个插值，着色点插值，用interpolate
                    ...
                    //将插值属性传入fragment_shader_payload
